...
OpenSpec schema 解析順序如下:

1. 專案層級 schema:`<project>/openspec/schemas/<name>/`
2. 使用者層級 schema:`${XDG_DATA_HOME:-$HOME/.local/share}/openspec/schemas/<name>/`
3. package 內建 schema

若希望本機所有專案都能使用 `superpowers-bridge`,用使用者層級安裝。若某個專案需要 pin 住或客製一份,用專案層級安裝。專案內若已存在 `openspec/schemas/superpowers-bridge/`,會遮蔽使用者層級安裝,直到你移除或升級那份本地 copy。
...
安裝後,任何專案都可以這樣開始 change:

```bash
/opsx:new my-feature --schema superpowers-bridge
```

如果專案已經有 `openspec/schemas/superpowers-bridge/`,也要升級那份專案層級 copy,或移除它讓使用者層級 schema 生效。

### 方法 2:專案層級 Claude Code 一鍵 prompt
...
### 方法 3:專案層級手動 bash(CI / 非 Claude 環境)
...
若你是使用者層級安裝,升級使用者層級目錄即可。若某個專案已經有 `openspec/schemas/superpowers-bridge/`,那份專案層級 copy 會遮蔽使用者層級安裝,必須另外升級或移除。

升級會整個覆蓋 `superpowers-bridge/` 目錄;專案層級安裝也會提供 CLAUDE.md 片段更新 — 詳情見下方「升級會【覆蓋】什麼?」。
...
### 升級方法 2:專案層級 Claude Code 一鍵 prompt
...
| 路徑 | 行為 | 是否需手動 |
|---|---|---|
| `openspec/schemas/superpowers-bridge/` | 自動整個目錄覆蓋 — 從 schema bundle 整包換新(專案層級手動方法是 `rm -rf` + `cp -R`;Claude Code prompt 等價) | 不需 |
...
> v1.1.1 的 graph tightening 不會讓 in-flight change 失效。如果已有 `brainstorm.md` 但還沒有 `proposal.md`, `/opsx:continue` 現在只會解鎖 `proposal.md`; `design.md` 會在 `proposal.md` 之後。既有 `design.md`、`verify.md`、`retrospective.md` 仍可讀。

> 未來若 schema graph 結構性變動(增刪 artifact、改 `requires:` edges、PRECHECK 變動),會列在下方「Known schema graph changes」。v1.x 純 instruction prose 改動仍安全,不需 migration。
...
| 反模式 | 為什麼錯 |
|---|---|
| schema 已安裝後仍讓 brainstorming 寫到 `docs/superpowers/specs/` | 繞過 [schema.yaml](./schema.yaml) 的 brainstorm redirection instruction,留下 orphan artifact |
| 讓 writing-plans 寫到 `docs/superpowers/plans/` | 繞過 [schema.yaml](./schema.yaml) 的 plan redirection instruction,留下 orphan artifact |
...
上方 Artifact DAG 顯示**檔案存在**依賴。Planning artifacts 是刻意串行:`proposal.md` 先界定 change scope,接著 `design.md`,而 `proposal.md` 與 `design.md` 都存在後才會解鎖 `specs/**/*.md`。下面這張完整 lifecycle 加上 apply phase 的順序步驟,以及 post-apply artifact 的**時序錯位**。
...
```text
PLANNING ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  brainstorm.md ──→ proposal.md ──→ design.md(必填) ──→ specs/**/*.md ──→ tasks.md ──→ plan.md
                                                                       │
...
> **時序註記**(完整理由見下方「設計觸點 #6」):
> - Planning order 由 `requires:` edges enforcement:`design.requires: [proposal]`,且 `specs.requires: [proposal, design]`。
...
> - 剩下的時序錯位只存在於 post-apply;planning order 不再依賴 `/opsx:continue` 在多個 ready artifacts 之間挑選。
...
### Brainstorm hardening(腦力激盪強化)

`brainstorm` artifact 仍然直接 invoke `superpowers:brainstorming`,但 schema 會加上一層 OpenSpec-safe 的強化 prompt,概念來自 grilling 與 domain-modeling 工作流:

- 對每個尚未解開的設計分歧,一次只問一個問題,並附上建議答案與取捨。
- 辨識 domain terms、identity、ownership boundary、lifecycle state、invariant、edge case 與 glossary conflict。
- 對可查證的假設,先看專案證據與官方文件,不要直接問使用者或憑記憶猜。
- 剩餘未知項必須寫成 explicit TBD,包含 owner、impact scope 與 blocking phase。

`brainstorm.md` 仍先保留 raw capture。raw transcript 之後附上 `## OpenSpec Capture Summary`,其中包含 Evidence Checked、Domain Model、Grilling Decision Tree、Resolved Decisions、Rejected Alternatives、Risks / Trade-offs、Documentation Candidates 與 Validated Direction。`proposal.md` 優先用這段 summary 產出 scope 與 capabilities。`design.md` 仍以 `brainstorm.md` 作為主要決策來源,只把 `proposal.md` 當成 scope / capability context。
...
Brainstorm hardening 也是 prompt-level 整合:不 vendoring、不修改第三方 skill。本 schema 仍只要求 `superpowers:brainstorming` 這個 brainstorm skill,然後要求模型在 raw capture 後附上可被下游 artifacts 穩定消費的 OpenSpec Capture Summary。
...
目前 bundle release: **`1.1.1`**(見 [VERSION](./VERSION))。
...
| Drift 通知 | [`version-check.yml`](../.github/workflows/version-check.yml) 每週,把基準 vs 最新 npm / GitHub release 字串比對 | Pinned ≠ latest upstream | 開 / 更新 [labelled drift issue](https://github.com/AlekseiSeleznev/openspec-schemas/issues?q=is%3Aopen+label%3Aupstream-version-check),由人類檢核(workflow 維持綠 — drift 是正常狀態,不是錯誤) |
...
- **v1.1.1** — 收緊 planning order,讓 graph 與文件化 lifecycle 一致:`design.requires` 從 `[brainstorm]` 改為 `[proposal]`,且 `specs.requires` 從 `[proposal]` 改為 `[proposal, design]`。Migration:不需要重寫既有檔案;in-flight change 會先補齊缺少的 predecessor 後繼續。
